...
import { fetchGetDataWithAuthArrayBuffer, fetchGetDataWithAuth, fetchDeleteDataWithAuth, fetchGetBlobDataWithAuth } from 'client/client.jsx';
    ...
    fetchGetDataWithAuth('/albums/' + album_id).then(res => {
      ...
      photosList.forEach(photo => {
        ...
        fetchGetDataWithAuthArrayBuffer('/'+thumbnail_link).then(response => {
          ...
          console.log("res2:", Buffer.from(response.data, 'binary'))
          console.log("res3:", Buffer.from(response.data, 'binary').toString('base64'))
          ...
          <Grid item key={key} xs={8} sm={4} md={4} lg={2}>
            <Card>
              <Tooltip title={photos[key]['description']}>
                <CardMedia component="img" height="200" image={'data:image/png;base64,' + photos[key]['content']} alt={photos[key]['description']}/>
              </Tooltip>
              <CardContent>
                <Tooltip title={photos[key]['description']}>
